...
def quaternion_phi_3_error(y_true, y_pred):
    return tf.acos(tf.abs(K.batch_dot(y_true, tf.math.l2_normalize(y_pred, axis=-1), axes=-1)))


def quaternion_phi_4_error(y_true, y_pred):
    return 1 - tf.abs(K.batch_dot(y_true, tf.math.l2_normalize(y_pred, axis=-1), axes=-1))


def quaternion_log_phi_4_error(y_true, y_pred):
    return tf.math.log(1e-4 + quaternion_phi_4_error(y_true, y_pred))
...
class CustomMultiLossLayer(Layer):
    ...
    def multi_loss(self, ys_true, ys_pred):
        ...
        precision = tf.exp(-self.log_vars[0][0])
        loss += precision * mean_absolute_error(ys_true[0], ys_pred[0]) + self.log_vars[0][0]
        precision = tf.exp(-self.log_vars[1][0])
        loss += precision * mean_absolute_error(ys_true[1], ys_pred[1]) + self.log_vars[1][0]
...
def create_pred_model_6d_quat(window_size=200):
    ...
    convC1 = Conv1D(128, 11)(poolA)
    convC2 = Conv1D(128, 11)(convC1)
    poolC = MaxPooling1D(3)(convC2)
    
    convD1 = Conv1D(128, 11)(poolB)
    convD2 = Conv1D(128, 11)(convD1)
    poolD = MaxPooling1D(3)(convD2)
    
    AB = concatenate([poolC, poolD])
...
def step_length_model(window_size=200):
    x1 = Input((window_size, 3), name='x1')
    x2 = Input((window_size, 3), name='x2')

    CD = concatenate([x1, x2])
    AB = Dense(128)(CD)
